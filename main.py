...
#   camera_bottom ──(qr_result_queue)──► core ──(SocketIO emit)──► React
#   camera_bottom ──(dock_event_queue)─► core ──(SocketIO emit)──► React
#
# Cara jalankan:
#   python main.py
#
...
def main():
    ...
    queues  = create_shared_queues(manager)

    qr_result_queue  = queues["qr_result"]
    dock_event_queue = queues["dock_event"]
    ...
    processes_config = [
        {
            ...
            "args":   (qr_result_queue, dock_event_queue),
        },
        {
            "name":   "CameraFront",
